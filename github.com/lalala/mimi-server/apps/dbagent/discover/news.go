...
// GetANews 获取一条新闻。
func (agt *DbDiscoverAgent) GetANews(ctx context.Context, arg *dbproto.NewsId) (*dbproto.News, error) {
	defer func() {
...
// QueryNewsListClient 客户端获取新闻列表
func (agt *DbDiscoverAgent) QueryNewsListClient(ctx context.Context, arg *dbproto.QueryNewsArg) (*dbproto.NewsList, error) {
	defer func() {
		if err := recover(); err != nil {
			log.Println("recover from panic:", err)
		}
...
func (agt *DbDiscoverAgent) ReadANews(ctx context.Context, arg *dbproto.NewsId) (*dbproto.Nil, error) {
	defer func() {
		if err := recover(); err != nil {
			log.Println("recover from panic:", err)
		}
