...
func convert_dtq3(sch *dbproto.BuycaiScheme, zfTypeId int, ballsFormat string) zhongfu.Scheme {
...
func validate_dtq3(sch *apiproto.BuycaiScheme) bool {
	danBalls, ok := validator.CheckBalls(sch, K_DAN, 1, 2, MIN, MAX)
	if !ok {
		return false
	}
	...
	numDan := len(danBalls)
	numTuo := len(tuoBalls)
	num := validator.Comb(numTuo, 3-numDan)
