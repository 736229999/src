...
func convert_jqzx4(sch *dbproto.BuycaiScheme, zfTypeId int, ballsFormat string) zhongfu.Scheme {
...
func validate_jqzx4(sch *apiproto.BuycaiScheme) bool {
	balls, ok := validator.CheckBalls(sch, K1, 4, 10, MIN, MAX)
	if !ok {
		return false
	}

	num := validator.Comb(len(balls), 4)
