    ...
    methods: {
          ...
          if (valid) {
            this.$confirm('确定提交了吗? 填错了会被老板骂哟！要不要在确认一下？', '提示', {
              ...
              type: 'error'
                ...
                acApi.updateActivity(this.form).then((res) => {
      ...
      GetGiftList:function () {
        ...
        acApi.allTask(this.form).then((res) => {
          this.checkList = res.data.msg.list;
        }).catch(err=>{
      ...
      GetActivityById: function () {
        ...
        acApi.editActivity({id: this.$route.params.id}).then((res) => {
          if (res.data.msg.taskLists == null){
            res.data.msg.taskLists = [];
          }
          this.form = res.data.msg;
          let moment = require("moment");
          this.form.starttime = moment(this.form.starttime * 1000).toDate();
          this.form.endtime = moment(this.form.endtime * 1000).toDate();
        })
      },
      //找到活动对应的所有任务
      GetTasksByActivityId: function () {
