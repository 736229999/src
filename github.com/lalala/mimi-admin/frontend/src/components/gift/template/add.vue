        ...
        <el-form-item label="购彩券">
          ...
          <div v-if="form.content.tickets.tickets.length > 0" id="showTickets">
            <el-table :data="form.content.tickets.tickets" style="width: 100%">
              ...
              <el-table-column prop="restrict_type_label" label="限制彩种"></el-table-column>
...
<script>
  ...
  export default {
    ...
    methods: {
        ...
        this.$refs.form.validate((valid) => {
          if (valid) {
            this.$confirm('确认提交吗?', '提示', {type: 'warning'}).then(() => {
              giftApi.addGift(this.form).then((res) => {
      ...
      //赠送积分.
      onCredits: function () {
        let credits = this.form.content.credits.credits;
        if (credits < 1) {
        ...
        console.log("赠送的积分:", this.form.content.credits.credits)
      },
...
//      },
      ...
      appendTickets: function () {
        ...
        if (restrict_type == "" && restrict_type != 0) {
          ...
          console.log(restrict_type, restrict_id);
          return false
        }
