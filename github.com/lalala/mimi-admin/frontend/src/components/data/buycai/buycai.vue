      ...
      <el-tab-pane label="按天统计" name="day">
        ...
        <el-col>
          <el-table :data="form.list" style="width: 100%" :default-sort="{prop: 'date', order: 'descending'}">
            <el-table-column prop="recharge_time" label="购彩日期" :formatter="formatter"></el-table-column>
            <el-table-column sortable label="总购彩金额/次数">
...
<script>
  import buycaiApi from "../../../api/buycai"
  import Vue from "vue"
  export default {
    data() {
      return {
      ...
      GetBuycaiByDay: function () {
        let moment = require("moment");
        buycaiApi.get(this.form).then((res) => {
